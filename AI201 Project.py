...
windows.minsize(750, 500)

myImg = Image.open(r'C:\Users\SLIM 7\Downloads\R.jpg')
resizedImg = myImg.resize((600, 400))
img = ImageTk.PhotoImage(resizedImg)
imgLabel = Label(image=img)
imgLabel.pack()

label1 = Label(text='WELCOME TO ROMANIA', font=("Times", 20))
label1.pack(pady=20)
label1.configure(bg='black', fg='white')

label2 = Label(text='Enter Your Current City', font=("Times", 12))
label2.pack(pady=10)
label2.configure(bg='black', fg='white')
current = Entry()
current.pack()

label3 = Label(text='Enter Your Destination', font=("Times", 12))
label3.pack(pady=10)
...
def greedy_button_on_click():
    start = current.get()
    end = target.get()
    path = str(greedy(graph, start, end))
    result = Label(text = "= "+path).pack()
...
def dfs_button_on_click():
    ...
    path = str(DFS(un_weighted_graph, start, end))
    result = Label(text = "= "+path).pack()

def ucs_button_on_click():
    start = current.get()
    end = target.get()
    path = str(UCS(graph, start, end))
    result = Label(text = "= "+path).pack()
        
    
def a_star_button_on_click():
    start = current.get()
    end = target.get()
    path = str(A_star(graph, start, end))
    result = Label(text = "= "+path).pack()

    

bfs_button = Button(windows, text="BFS", command=bfs_button_on_click)
bfs_button.pack()

dfs_button = Button(windows, text="DFS", command=dfs_button_on_click)
dfs_button.pack()

ucs_button = Button(windows, text="UCS", command=ucs_button_on_click)
ucs_button.pack()

a_star_button = Button(windows, text="A*", command=a_star_button_on_click)
a_star_button.pack()

greedy_button = Button(windows, text="Greedy", command=greedy_button_on_click)
greedy_button.pack()
